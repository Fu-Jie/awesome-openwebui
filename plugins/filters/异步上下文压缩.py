"""
Open WebUI Filter: 异步上下文压缩过滤器（数据库持久化版）
...
keep_first (保留初始消息数)
  默认: 1
  说明: 始终保留对话开始的 N 条消息。设置为 0 则不保留。第一条消息通常包含重要的系统提示或环境变量。
...
删除过期摘要：
  DELETE FROM openwebui_chat_summaries
...
1. 前置依赖
   ⚠ 本过滤器假设已有其他过滤器负责注入用户环境变量。
   ⚠ 确保环境变量过滤器的 priority 小于本过滤器。

2. 数据库权限
   ⚠ 确保 `DATABASE_URL` 指向的用户有建表权限。
   ⚠ 首次运行会自动创建 `openwebui_chat_summaries` 表。
...
问题：环境变量丢失
解决：
  1. 确认环境变量过滤器在本过滤器之前执行。
  2. 检查 `priority` 设置是否正确。
  3. 验证环境变量注入是否成功。
...
changelog:
  v2.2.0 - 移除 enable_summary 配置
  v2.1.0 - 增加灵活的保留策略 (keep_first, keep_last)
  v2.0.0 - 增加 SQLite 支持并移除内存存储
  v1.0.0 - 初始版本，支持内存存储和智能摘要
...
class ChatSummary(Base):
    ...
    __tablename__ = "openwebui_chat_summaries"
...
class Filter:
    ...
    def _init_database(self):
        ...
        try:
            ...
            print(f"[数据库] ✅ 成功连接到 {db_type} 并初始化 openwebui_chat_summaries 表")
    ...
    class Valves(BaseModel):
        ...
        compression_threshold: int = Field(
            default=15, description="触发压缩的消息数阈值"
        )
        # 保留策略
